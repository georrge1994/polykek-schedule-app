 ...
 *
 * @constructor Create [SwipeToDeleteCallback]
 *
 * @param context Context
 */
internal abstract class SwipeToDeleteCallback(context: Context) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
    ...
    ): Boolean = false // We don't want support moving items up/down.
     ...
     * @return Alpha
     */
    private fun getAlpha(dX: Float): Int {
        val value = (abs(dX) - intrinsicWidth)
        return if (value <= 255) {
            value.toInt()
        } else {
            255
